...
public class ScanResultAdapter extends BaseAdapter {
    ...
    public View getView(int position, View view, ViewGroup parent) {
        ...
        String rssi = String.valueOf(scanResult.getRssi());

        deviceNameView.setText("Device Name: " + name);
        deviceAddressView.setText("Device Mac: " + scanResult.getDevice().getAddress());
        deviceRssiView.setText("Device Rssi: " + rssi);
        lastSeenView.setText(getTimeSinceString(mContext, scanResult.getTimestampNanos()));

        // add by tjdai, set to red when the distance is less than 1 meter
        if (scanResult.getRssi() >= rssiA)
        {
            deviceNameView.setTextColor(Color.RED);
            deviceAddressView.setTextColor(Color.RED);
            deviceRssiView.setTextColor(Color.RED);
            lastSeenView.setTextColor(Color.RED);
        }
        else
        {
            deviceNameView.setTextColor(Color.BLACK);
            deviceAddressView.setTextColor(Color.BLACK);
            deviceRssiView.setTextColor(Color.BLACK);
            lastSeenView.setTextColor(Color.BLACK);
        }
